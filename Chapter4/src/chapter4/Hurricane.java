/*

Program: Hurricane.java          Last Date of this Revision: 05-Mar-2022

Purpose: Create a Hurricane application that displays the wind speed for the hurricane category entered by the user.

Author: Ashleen Sidhu, 
School: CHHS
Course: Computer Programming 20
 
*/

package chapter4;

import java.util.Scanner;

public class Hurricane 
{
	public static void main(String[] args)
	{
		int category;
		Scanner input = new Scanner(System.in);
		
		System.out.print("Enter category of the hurricane: ");//user enters category
		category = input.nextInt();
		input.close();
		
		switch(category) //categories of hurricanes 
		{
		case 1: System.out.println("Category 1: 74-95 mph or 64-82 kt or 119-153 km/hr"); break;
		
		case 2: System.out.println("Category 2: 96-110 mph or 83-95 kt or 154-177 km/hr"); break;
		
		case 3: System.out.println("Category 3: 111-130 mph or 96-113 kt or 178-209 km/hr"); break;
		
		case 4: System.out.println("Category 4: 131-155 mph or 114-135 kt or 210-249 km/hr"); break;
		
		case 5: System.out.println("Category 5: greater than 155 mph or 135 kt or 249 km/hr"); break;
		}
	}

}

/* Screen Dump 

Enter category of the hurricane: 1
Category 1: 74-95 mph or 64-82 kt or 119-153 km/hr

Enter category of the hurricane: 2
Category 2: 96-110 mph or 83-95 kt or 154-177 km/hr

Enter category of the hurricane: 3
Category 3: 111-130 mph or 96-113 kt or 178-209 km/hr

Enter category of the hurricane: 4
Category 4: 131-155 mph or 114-135 kt or 210-249 km/hr

 */